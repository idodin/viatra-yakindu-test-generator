...
import mapper.transform.Yakindu2GMFMapper
import mapper.transform.YakinduNamer
import mapper.util.CombinedUtil
...
//		var reasoner = new AlloySolver
//		val vampireConfig = new AlloySolverConfiguration => [
//			// add configuration things, in config file first
////			it.documentationLevel = DocumentationLevel::FULL
//			it.solutionScope = ss
//			it.typeScopes.maxNewElements = 10
...
//				].isEmpty
		...
		println("Generating Yakindu Concrete Syntax")
		...
		for(modelIndex : 0..<models.size) {
				...
				val root = YakinduNamer.nameElements(Collections.singletonList(model));
				CombinedUtil.saveModel(root, Yakindu2GMFMapper.generateConcrete(root), '''«outWS»/combined«representationNumber».sct''')
			}
		println("Yakindu Models Generated")
	}
